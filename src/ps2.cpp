...
void PS2::init() {
	...
	IO::out(cmdPort, (u8)Command::TestPort1);
	TestPortResult status = (TestPortResult)IO::in(dataPort);
	...
	ResetResponse resetResponse = (ResetResponse)IO::in(dataPort);
	isResetWorking = resetResponse == ResetResponse::Success;
}

void PS2::enablePort(bool first) {
	if (first) {
		IO::out(cmdPort, (u8)Command::EnablePort1);
	} else {
		IO::out(cmdPort, (u8)Command::EnablePort2);
	}
}

void PS2::disablePort(bool first) {
	if (first) {
		IO::out(cmdPort, (u8)Command::DisablePort1);
	} else {
		IO::out(cmdPort, (u8)Command::DisablePort2);
	}
}

bool PS2::test() {
	IO::out(cmdPort, (u8)Command::TestController);
	TestResult res = (TestResult)IO::in(dataPort);
